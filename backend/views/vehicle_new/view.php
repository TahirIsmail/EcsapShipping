...
Notification::widget([
    ...
    <?php
                ...
                if (isset($Role['customer'])) {
                    ...
                    ?>
    <?= Html::button(Yii::t('app', 'Update'), ['value'=>Yii::$app->urlManager->createUrl('vehicle/update?id='.$model->id),'class' => 'btn btn-primary click_modal', 'data-toggle' => 'tooltip', 'data-placement' => 'bottom',]) ?>
    <?php   echo Html::a("Update",[''],[
                'value'=>Yii::$app->urlManager->createUrl('vehicle/update?id='.$model->id), //<---- here is where you define the action that handles the ajax request
                'class'=>'click_modal grid-action btn btn-primary click_modal',
                'data-toggle'=>'tooltip',
    ...
    <tr><th>Status</th><td><?php if($model->status){ echo \common\models\Lookup::$status[$model->status]; } ?></td></tr>
