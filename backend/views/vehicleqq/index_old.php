...
//for coulumn colors same like asl
            ...
            if ($model->status == 1) {
                ...
                return ['style' => 'background-color:#FFFF66']; //MANIFEST
            } else if ($model->status == 3) {
                return ['style' => 'background-color:#DF9D9D']; //ON THE WAY
            ...
            [
              ...
              'model',
               ...
               [
                ...
                'format' => 'raw',
                'value' => function ($model) {
                    return Html::button(Yii::t('app', $model->lot_number), ['value' => Yii::$app->urlManager->createUrl('vehicle/conditionreport?id='.$model->id), 'class' => 'click_modal_report', 'style' => 'border:none;background:none;color:#337ab7;']);
                },
            ],
     //        [
...
//                        $p = $model->towingRequest->towing_request_date;
        ...
        'value' => function ($model) {
         ...
         'filter' => [1 => 'ON HAND', 2 => 'MANIFEST', 3 => 'ON THE WAY', 4 => 'SHIPPED',5 => 'PICKED UP',6 => 'ARRIVED', '' => 'No Status'],
     ],
    [
        'label' => 'Manifest Date',
...
return Html::a($model->container_number, Yii::$app->urlManager->createUrl('/export/view?id='.$vehicle_container['export_id'].$model->container_number));
    ...
    // ],
     ...
     //'towingRequest.towed:boolean',
      ...
      ['class' => 'yii\grid\ActionColumn',
                ...
                'template' => '{manifest}{bl}{update}{docs}{view}{delete}',
                'buttons' => [
                    'manifest' => function ($url, $model, $key) {
                        $export_id = 0;
                        ...
                        $btn = Html::a("<span class='glyphicon glyphicon-file'></span>", [''], [
                            'value' => Yii::$app->urlManager->createUrl('/export/manifestmodal?id=' . $export_id), //<---- here is where you define the action that handles the ajax request
                            'class' => 'click_modal_report grid-action ',
                            'data-toggle' => 'tooltip',
                            'data-placement' => 'bottom',
                            'title' => 'Manifest',
