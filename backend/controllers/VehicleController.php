...
class VehicleController extends Controller
{
    ...
    public function actionExportExcel()
    {
        try {
            $searchModel = new VehicleSearch();
            $dataProvider = $searchModel->search(Yii::$app->request->queryParams, 5000);
            $models = $dataProvider->getModels();
            $excelArray = array();
            foreach ($models as $model) {
                $data = new \common\models\helpers\VehicleExcel();
                $data->total_photos = count(\common\models\Images::find()->where(['=', 'vehicle_id', $model->id])->all());
                ...
                $data->title_recieved = $model->towingRequest->title_type;
                $data->buyer_id = $model->license_number;
                $data->keys = $model->keys == 1 ? 'Yes' : 'No';
                $data->loc = isset(\common\models\Lookup::$location[$model->location]) ? \common\models\Lookup::$location[$model->location] : $model->location;
                ...
                $data->manifest_date = isset($vehicle_export['export']['created_at']) ? date('Y-m-d', strtotime($vehicle_export['export']['created_at'])) : '';
    ...
    public function actionFrontsearch()
    {
        ...
        $vehicle_detail = Vehicle::find()->orWhere(['or', ['vin' => $vin], ['lot_number' => $vin]])->asArray()->one();
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $towing->load(Yii::$app->request->post()) || $vehiclefeatures->load(Yii::$app->request->post()) && $vehiclecondition->load(Yii::$app->request->post()) && $images->load(Yii::$app->request->post()) && $docs->load(Yii::$app->request->post())) {
            $model->pieces = 1;
            if(isset(Yii::$app->request->post('Vehicle')['customer_user_id'])) {
                $model->customer_user_id = Yii::$app->request->post('Vehicle')['customer_user_id'];
            }
    ...
    public function actionUpdate($id)
    {
        ...
        if ($docs_old) {
            foreach ($docs_old as $doc) {
                $baseurl = \Yii::$app->request->BaseUrl;
                $doc_url = $baseurl.'/uploads/'.$doc->thumbnail;
                $all_docs[] = Html::img("$doc_url", ['class' => 'file-preview-image']);
                $obj = (object) array('caption' => '', 'url' => $baseurl.'/vehicle/delete-image', 'key' => $doc->id);
    ...
    public function actionDownloadImages($id)
    {
        ...
        $file = 'vehicleimages.zip';
        if (file_exists('uploads/'.$file)) {
            unlink('uploads/'.$file);
        }

        $zip = new ZipArchive();
        if ($zip->open('uploads/'.$file, ZipArchive::CREATE) !== true) {
        ...
        $zip->close();
        header('Content-disposition: attachment; filename="vehicleimages.zip"');
    ...
    {
        $vehicle_detail = Vehicle::find()->orWhere(['or', ['vin' => $vin_id], ['lot_number' => $vin_id]])->one();
        ...
        if ($vehicle_detail->location) {
            $v_location = \common\models\Lookup::$location[$vehicle_detail->location];
        } else {
    ...
    public function actionMpdf($id, $mail = null)
    {
        ...
        $pdf = new Pdf([
            ...
            'options' => [
                ...
                'subject' => 'Generated PDF By AFG Global SHIPPING',
            ],
            'methods' => [
        ...
        if ($mail) {
            ...
            if ($customerMail) {
                ...
                $sendemail = Yii::$app->mailer->compose()
                    ...
                    ->setTo($customerMail)
                    ->setSubject('CONDITION REPORT')
                    ->setHtmlBody('Dear valued customer please see the attached condition report for your vehicle.* 
                    <br><br>Thanks,<br> AFG Global Shipping Inc</br>
                    *Please read following terms and condition:<br/>
                    *Vehicle Condition should be checked and compared as soon as vehicle reched in USA yard,
                    *AFG Global world wide will not be liable for any missing (Keys, CD player, Navigation etc.) or
                    for any damage after it arrives in destination which is already mentioned in condition report.<br/>
                    *Under vehicle damage will not be accepted.
                    ')
                    ->send();
    ...
    //added on 22122020 | email to customer
    public function actionEmailtocustomer($id){
        ...
        $lot_number = $vehicleDetail->lot_number;

        $sendemail = Yii::$app->mailer->compose()
                    ->setFrom([\Yii::$app->params['supportEmail'] => 'AFG Global Shipping'])
                    ->setTo($customerMail)
                    ->setSubject('Unit Information / '.$year.', '.$make.', '.$model.'')
                    ->setHtmlBody('Dear Customer, <br><br>
                     Your vehicle images has been uploaded in the system. Please check the Images, Keys, and title status. 
                    <br><br>'.$year.' '.$make.' '.$model.'
                    <br><b>VIN: </b>'.$vin.'</br>
                    <br><b>LOT#: </b>'.$lot_number.'
                    <br><br>Thank you for becoming a customer of AFG Global Shipping.<br>
        ...
        //return json_encode($sendemail);
    }
    public function actionGetexport_vehicle($id, $export_id)
    {
        $all_vehicles = Vehicle::find()->select('vehicle.*')->join('join vehicle_export on vehicle.id = vehicle_export.vehicle_id', [])->where(['=', 'export_id', $export_id])
    ...
    public function actionDelete($id)
    {
        ...
        try {
            ...
            $message = $user->username.' has deleted Vehicle with VIN '.$model->vin;
